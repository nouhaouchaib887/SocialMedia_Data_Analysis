...
class TiktokAPifyCollector:
    ...
    def _transform_post(self, post):
        ...
        return {
             "post_id": post.get("id"),
            "source_id": post.get("authorMeta").get("id"),  # ID de l'utilisateur/page qui a posté
            "created_time": post.get("createTimeISO"),
            "permalink": post.get("webVideoUrl"),
            "page_id":  post.get("authorMeta").get("id"),  # Même que source_id
            "page_name": post.get("authorMeta").get("name") ,  # Nom de la page
            "message": post.get("text", ""),
            "media_type":  "photo" if post.get("videoMeta", {}).get("duration", 0) == 0 else "video",
            "media_url": post.get("webVideoUrl"),
            "thumbnail_url": post.get("videoMeta").get("coverUrl"),
            "can_share": True,
            "shares": post.get("shareCount", 0),  # Champ correct pour les partages
            "can_comment": True,
            "comments_count": post.get("commentCount", 0),  # Champ correct pour les commentaires
            "can_like": True,
            "like_count": post.get("diggCount", 0),  # Champ correct pour les likes
    ...
    def _transform_comment(self, comment):
        ...
        return {
            ...
            "user_name": comment.get("uniqueId"),
            "user_url":  f"https://www.tiktok.com/@{comment.get('uniqueId')}/" if comment.get("uniqueId") else None,
    ...
    def collect_posts(self):
        ...
        """
        print(f"📱 Collecte des posts de {self.profile_name} depuis {self.from_date}")
        
        actor_input = {
            "excludePinnedPosts": False,
    #"oldestPostDateUnified":self.from_date ,
    "profiles": [
        self.profile_name
    ],
