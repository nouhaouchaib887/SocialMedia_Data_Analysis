...
class InstagramAPifyCollector:
    ...
    def _transform_post(self, post):
        ...
        return {
    "post_id": post.get("id"),
    "source_id": post.get("ownerId"),  # ID de l'utilisateur/page qui a posté
    "created_time": post.get("timestamp"),
    "updated_time": None,
    "permalink": post.get("url"),
    "page_id": post.get("ownerId"),  # Même que source_id
    "page_name": post.get("ownerUsername") ,  # Nom de la page
    "message": post.get("caption", ""),
    "media_type":  post.get("type"),
    "media_url": post.get("videoUrl") or post.get("url"),
    "thumbnail_url": post.get("displayUrl",""),
    "can_share": True,
    "shares": post.get("shares", 0),  # Champ correct pour les partages
    "can_comment": not post.get("isCommentsDisabled"),
    "comments_count": post.get("commentsCount", 0),  # Champ correct pour les commentaires
    "can_like": True,
    "like_count": post.get("likesCount", 0),  # Champ correct pour les likes
    ...
    def _transform_comment(self, comment):
        ...
        return {
            ...
            "user_id": comment.get("owner", {}).get("id"),
            "user_name": comment.get("ownerUsername"),
            "user_url":  f"https://www.instagram.com/{comment.get('ownerUsername')}/" if comment.get("ownerUsername") else None,
    ...
    def collect_posts(self):
        ...
        """
        print(f"📱 Collecte des posts de {self.user_name} depuis {self.from_date}")
