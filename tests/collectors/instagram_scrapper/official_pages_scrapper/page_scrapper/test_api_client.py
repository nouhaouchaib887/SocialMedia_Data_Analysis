...
from src.collectors.instagram_scrapper.official_pages_scrapper.page_scrapper.api_client import *
...
class TestApifyAPIClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = ApifyInstagramClient(self.apify_token)
        self.base_url = self.client.base_url
        self.actor_id = self.client.actor_id

    def test_init_success(self):
        """Test l'initialisation réussie du client"""
        client = ApifyInstagramClient("some_token")
        self.assertEqual(client.apify_token, "some_token")
        self.assertEqual(client.actor_id, 'apify~instagram-profile-scraper')
    ...
    @patch.object(ApifyInstagramClient, '_normalize_instagram_url', side_effect=lambda x: f"normalized_{x}")
    ...
    @patch('requests.post')
    @patch.object(ApifyInstagramClient, '_prepare_start_urls')
    def test_start_scraping_run_success(self, mock_prepare_urls, mock_requests_post):
        ...
        urls_to_scrape = ["http://instagram.com/page"]
    ...
    @patch('requests.post')
    @patch.object(ApifyInstagramClient, '_prepare_start_urls', return_value=[])
    ...
    @patch('requests.post')
    @patch.object(ApifyInstagramClient, '_prepare_start_urls', return_value=[{"url": "url", "method": "GET"}])
    ...
    @patch.object(ApifyInstagramClient, 'get_run_status')
    @patch('time.sleep')
