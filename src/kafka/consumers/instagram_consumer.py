...
class InstagramDataConsumer:
    """Consumer Kafka pour traiter les données Instagram"""
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.consumer = KafkaConsumer(
                self.kafka_config.get('topic', 'instagram-data'), # Use get with default
                ...
                group_id=self.kafka_config.get('group_id', 'instagram-data-consumer'), # Use get
            ...
            db = self.mongo_client[self.mongodb_config.get('database', '')] # Use get
        ...
        try:
            ...
            if existing_post:
                ...
                post_document = {
                    ...
                    if key != "comments" and key!= "kafka_metadata" and key!= "collection_params" # Exclure les commentaires imbriqués
                }
                post_document["updated_time"] = None
    ...
    async def process_comment(self, comment_data: Dict[str, Any],
        ...
        try:
            ...
            comment_document = {
                ...
                if key != "comment_metadata"  # Exclure les commentaires imbriqués
                }
            #comment_document = comment_data.copy()
            comment_document["parent_post_id"] = comment_data.get("comment_metadata","").get("parent_post_id","")
    ...
    async def run(self):
        ...
        logger.info("Démarrage du consumer Instagram Data...")
...
async def main():
    ...
    kafka_config = {
        ...
        'group_id': 'instagram-data-consumer-group',
        'topic': 'instagram-data',
    ...
    # Récupération du token gemini
    google_api_key =config.get("api_keys", {}).get("gemini_key", "")

    # Create the consumer instance
    consumer = InstagramDataConsumer(kafka_config, mongodb_config, google_api_key)
